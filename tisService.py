...
class TisService(CasService):
    ...
    def querySchedule(self, date):
        ...
        response = self.session.post(
            "https://tis.sustech.edu.cn/component/queryrcxxlist",
            headers=headers,
            data=data,
        )
    ...
    def queryScheduleInterval(
        ...
        dates_to_fetch = [
            start_date + i * delta for i in range((end_date - start_date).days + 1)
        ]
        ...
        for d in dates_to_fetch:
            ...
            if (
                skip_holidays
                and holiday_provider is not None
                and holiday_provider.is_holiday(day)
            ):
                result[day] = []
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            future_to_date = {
                executor.submit(self.querySchedule, d.strftime("%Y-%m-%d")): d.strftime(
                    "%Y-%m-%d"
                )
                for d in dates_for_request
            }
        ...
        # 按日期排序，确保结果有序
        sorted_result = {
            d.strftime("%Y-%m-%d"): result.get(d.strftime("%Y-%m-%d"), [])
            for d in dates_to_fetch
        }
...
if __name__ == "__main__":
    ...
    service.Login()
    if service.LoginTIS():
        schedule = service.queryScheduleInterval("2026-04-20", "2026-05-02")
